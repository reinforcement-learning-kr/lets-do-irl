...
trejectories = np.load(file="expert_trajectories.npy")
...
def subtract_policyList(policyList):
    #infeasible할시에 맨위 FE를 list에 제외
...
def plot_score(scores, epi):

    fig,ax = plt.subplots(1,1,  figsize=(10, 10))
    
    plt.title('Scores in episode : {}'.format(epi))
    ...
    plt.draw()
    plt.savefig('Scores in episode_{}.png'.format(epi))
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    W, status = QP_optimizer(expert_feature_expectation_List, agent_feature_expectation_List)
    ...
    while True:
        ...
        while True:
            ...
            if done:
                scores.append(score)
                if episode >= 35000:#np.mean(scores[-min(10, len(scores)):]) > -120:
        ...
        if episode % 5000 == 0:
            ...
            while status=="infeasible":
                ...
                if status=="infeasible":
                    agent_feature_expectation_List = subtract_policyList(agent_feature_expectation_List)
